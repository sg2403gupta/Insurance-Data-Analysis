...
An end-to-end Insurance Analytics project covering data preparation,
SQL-based business analysis, and interactive Power BI reporting.

The project is designed to demonstrate a practical Data Analyst /
Business Intelligence workflow: starting with raw insurance, customer,
employee, and vendor data, validating and cleaning the data, performing
business-focused SQL analysis, and presenting the results through an
interactive Power BI dashboard with customer-level drill-through.

📌 Project Overview

This project analyzes an insurance dataset to understand:
...
The project combines Excel, SQL Server, Power BI, Power Query, DAX,
and data visualization into one complete analytics workflow.

🎯 Business Objectives

The analysis is designed to answer practical insurance-business
questions such as:
...
Which customers have unusually high claim exposure or loss ratios?

How can a manager drill from portfolio-level metrics to an
individual customer?
...
The raw data is maintained separately from the cleaned datasets.
...
The cleaned files are used as the analysis-ready layer before loading
data into SQL Server and Power BI.

The project separates raw data from cleaned data so that the
original source remains preserved and the transformation stage is
reproducible.

🗄️ 2. SQL Server Analysis

SQL Server is used for data validation, business analysis,
customer/vendor analysis, claims analysis, risk analysis, and advanced
analytical queries.

The SQL work is divided into five logical scripts.

Creating-Database-and-View-All-Data.sql

This script focuses on initial data validation and quality checks,
including:
...
This script calculates core insurance KPIs, including:
...
These measures form the analytical foundation for the Power BI executive
and claims dashboards.

📊 3. Insurance Claims & Risk Analytics

Insurance Claims & Risk Analytics.sql focuses on insurance-specific
claims and risk questions.
...
👥 4. Customer & Vendor Analytics

Customer & Vendor Analytics.sql contains three analytical sections.
...
This demonstrates practical use of INNER JOIN, GROUP BY, HAVING,
aggregation, and multi-dimensional analysis.

🧠 5. Advanced SQL Analytics

Advanced_SQL_Anlytics_.sql demonstrates advanced SQL techniques used
for deeper analytical questions.
...
ROW_NUMBER() is used with PARTITION BY to identify the top five
customers for each insurance type.
...
LAG() is used to compare the current month's claim exposure with the
previous month.
...
Claims reported more than seven days after the loss date are identified
and the overall late-reporting percentage is calculated.

📈 6. Power BI Dashboard

The Power BI report contains six analytical pages, progressing from
executive-level monitoring to customer-level investigation.

The dashboard uses slicers, KPI cards, charts, tables, and drill-through
navigation to support interactive analysis.
...
The page also provides filters for:
...
The report shows a customer population of approximately 10K in the
displayed overall view, with an average age of 44, average family
size of 4, and average tenure of 63.
...
The transaction table exposes fields such as:
...
The displayed report shows approximately 1K high-risk customers and
provides a direct comparison of claim exposure across Low, Medium, and
High risk segments.
...
This creates a drill-down path from portfolio-level analysis to an
individual customer and incident profile.

📌 Key Analytical Areas

The project covers the following business dimensions:
...
This validation layer helps reduce the risk of producing misleading
business insights from inconsistent records.

⚠️ Data Interpretation Note

The dashboard currently displays claim amounts that are substantially
larger than premium amounts in the overall dataset. For example, the
displayed overall view contains approximately $885.09K in premium
versus $165.64M in claims.

Because this produces an unusually high claim-to-premium relationship,
the monetary units and source-data definitions should be validated
before using the figures for real-world insurance profitability
conclusions.

The project therefore demonstrates the analytical workflow and BI
implementation, while financial interpretation should be made only
after confirming the source-data units and business definitions.
...
Perform data-cleaning and preparation as represented by the files in:

Cleaned_Data/

Step 2 --- Load the data into SQL Server

Create/import the required tables and load the cleaned insurance,
customer/vendor-related data.

Run:

SQL/Creating-Database-and-View-All-Data.sql

to perform the initial validation checks.
...
These scripts progressively move from basic KPIs to advanced analytical
queries.
...
This project demonstrates how raw insurance data can be transformed into
...
high-level KPIs → segment analysis → risk analysis → transaction
details → individual customer investigation.

📁 Repository Organization

The repository intentionally separates each stage of the analytics
lifecycle:
...
This structure makes the project easier to understand, review,
...
Insurance Analytics is an end-to-end business intelligence project
that combines Excel data preparation, SQL Server analytics, and Power
BI dashboarding to analyze insurance premiums, claims, customers,
vendors, risk, incidents, and geographic exposure.

The project demonstrates a complete analyst workflow from raw data →
cleaned data → validated database → analytical SQL → interactive BI
dashboard → customer-level drill-through.
